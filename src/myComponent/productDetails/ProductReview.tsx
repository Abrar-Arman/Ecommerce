...
function ProductReview({ review }: TProductReviewProps) {
  ...
  return (
    <div className="mt-12">
      <h2 className="uppercase text-[#343434] text-3xl">Review</h2>
        ...
        {review.map((rev, idx) => (
          <li key={idx} className="w-[80%] shadow  pl-3 py-6">
            <div className="flex items-center gap-2">
              <h3 className="text-2xl font-Prata font-bold">{rev["reviewerName"] as string}</h3>
            ...
            <p className="mt-2 text-lg">{rev["comment"] as string}</p>
          </li>
        ))}
      </ul>
    </div>
